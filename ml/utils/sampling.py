...
def create_dance_quartiles(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def assign_dance_q(group):
        ...
        if n_unique >= 4:
            # Use quartiles
            group["dance_q"] = pd.qcut(
                danceability, q=4, labels=[1, 2, 3, 4], duplicates="drop"
            )
        elif n_unique >= 3:
            # Fall back to tertiles
            group["dance_q"] = pd.qcut(
                danceability, q=3, labels=[1, 2, 3], duplicates="drop"
            )
        elif n_unique >= 2:
            # Fall back to halves
            group["dance_q"] = pd.qcut(
                danceability, q=2, labels=[1, 2], duplicates="drop"
            )
        else:
...
def stratified_sample(
    df: pd.DataFrame, n: int, seed: int, min_per_stratum: int = 15
    ...
    capped_groups = []
    for (year_bin, genre_bucket), group in combined_sample.groupby(
        ["year_bin", "genre_bucket"]
    ):
    ...
    if current_size > n:
        ...
        if len(available_for_backfill) > 0:
            backfill_strata = (
                available_for_backfill.groupby(strata_cols)
                .size()
            ...
            for stratum, available_count in backfill_strata.items():
                ...
                if room_in_cap > 0:
                    ...
                    if sample_size > 0:
                        backfill_sample = stratum_df.sample(
            ...
            if backfill_samples:
                ...
                combined_sample = pd.concat(
                    [combined_sample, backfill_df], ignore_index=True
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Stratified sampling for Spotify features"
    )
    parser.add_argument(
        "--in", dest="input_path", required=True, help="Input parquet file path"
    )
    parser.add_argument(
        "--out",
        dest="output_path",
    ...
    parser.add_argument(
        "--n", type=int, default=50000, help="Target number of samples (default: 50000)"
    )
    parser.add_argument(
    ...
    if len(df) < args.n * 0.8:
        print(
            f"❌ Only {len(df):,} tracks available, cannot meet 80% of target {args.n:,}"
        )
        sys.exit(1)
    ...
    for _, row in top_strata.iterrows():
        print(
            f"  {row['year_bin']} × {row['genre_bucket']} × Q{row['dance_q']}: {row['count']}"
        )
    ...
    print("\n🎉 Sampling complete!")
    print(
        f"Manifest: {len(manifest_df):,} tracks, {len(available_output_cols)} columns"
    )
