...
def get_spotify_preview(session: requests.Session, track_id: str, track_name: str, artist_name: str) -> dict:
    """
    ...
    try:
        ...
        response = session.get(
            f"http://localhost:8000/predict/spotify_track",
...
def get_deezer_preview_by_isrc(session: requests.Session, isrc: str) -> dict:
    """
...
def get_deezer_preview_by_search(session: requests.Session, track_name: str, artist_name: str) -> dict:
    """
...
def download_preview(session: requests.Session, preview_url: str, output_path: Path) -> dict:
    """
...
def process_track(track_data: dict, previews_dir: Path) -> dict:
    """
    ...
    if not preview_result or not preview_result.get("preview_url"):
        return {
            ...
            "file_size": 0,
            "file_path": None,
            "error_message": preview_result["error"] if preview_result else "No preview URL found"
        }
    
    # Download the preview
    download_result = download_preview(session, preview_result["preview_url"], output_path)
    ...
    return {
        ...
        "file_path": str(output_path) if download_result["status"] == "success" else None,
        "error_message": download_result["error"]
    }


def main():
    parser = argparse.ArgumentParser(description="Fetch preview MP3s for manifest tracks")
    parser.add_argument(
        "--manifest", 
        ...
        for future in as_completed(future_to_track):
            ...
            try:
                ...
                if completed % 100 == 0 or completed == len(tracks):
                    ...
                    print(f"  Progress: {completed:,}/{len(tracks):,} ({completed/len(tracks)*100:.1f}%) - {rate:.1f} tracks/sec")
    ...
    source_counts = results_df[results_df['preview_status'] == 'success']['preview_source'].value_counts()
    ...
    # File size stats
    successful_downloads = results_df[results_df['download_status'] == 'success']['file_size']
